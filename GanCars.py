    ...
    def train(self, epochs, batch_size=10, sample_interval=50):
        ...
        for dirName, subdirList, fileList in os.walk(rootDir):
            ...
            for fname in fileList:
                ...
                # print(np.array(Image.open(dirName + "/" + fname)).shape)
                X_train.append((np.array(Image.open(dirName + "/" + fname))))
                y_train.append((np.array(os.path.basename(dirName))))

        X_train = np.array(X_train)
        y_train = np.array(y_train)

        # Configure input
        #X_train = (X_train.astype(np.uint8) - 127.5) / 127.5
